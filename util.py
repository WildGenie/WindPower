...
def Compare_dev_result(dm,dir):
    ...
    new_period = np.array([f'{s}_{str(i).zfill(2)}' for s in new_season for i in range(1, 20 + 1)])
    new_field = np.array([
        'WindField_1',
        'WindField_2'
    ])
    ...
    for f in range(field_len):
        ...
        for m in tqdm(range(machine_len)):
            ...
            for p in range(period_len):
                ...
                visualize_prediction_compare(dm.X0[f, dm.val_indexes[field[f]][0][p], m],dm.Y0[f, dm.val_indexes[field[f]][0][p], m],Y_pre = var_pred[i*20:(i+1)*20,:],dir_name=os.path.join(dir, new_field[f], machine[f][m]), png_name=new_period[p])